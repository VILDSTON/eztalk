...
const PRESET_BANNERS = [
  ...
  'linear-gradient(135deg, #05140b 0%, #004d25 50%, #00ff73 100%)',
...
export const EditProfileModal: React.FC<EditProfileModalProps> = ({
  ...
  const [accentColor, setAccentColor] = useState(currentUser.accentColor || '#00ff73');
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setAccentColor(currentUser.accentColor || '#00ff73');
  ...
  // Check if handle is already taken by ANOTHER contact
  const isHandleTaken =
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-fade-in select-none p-4">
      <div className="bg-[#15161b] border border-[#2b2d36] rounded-3xl w-full max-w-lg shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
        ...
        {/* Modal Top Bar */}
        <div className="px-6 py-4 border-b border-[#252731] flex items-center justify-between shrink-0 bg-[#17181e]">
          <div>
            <h3 className="text-base font-bold text-white tracking-tight">Customize Profile</h3>
            <p className="text-[11px] text-gray-400">Personalize your avatar, banner, and presence</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSave} className="overflow-y-auto custom-scrollbar p-6 space-y-4 pr-3">
          ...
          <div className="rounded-2xl border border-[#282a36] overflow-hidden bg-[#121317]">
            {/* Banner Header */}
            <div className="h-24 w-full relative transition-all duration-300 flex items-end justify-between p-2.5" style={{ background: banner }}>
            ...
            {/* Avatar & Upload Row */}
            <div className="p-4 flex items-center justify-between bg-[#131419]">
              <div className="flex items-center space-x-3.5">
                <div className="relative group shrink-0">
                  <div
                    className="w-14 h-14 rounded-full overflow-hidden border-2 border-[#00ff73] bg-gray-800 shadow-md shrink-0"
                ...
                <div>
                  ...
                  <p className="text-xs font-mono text-[#00ff73]">{formattedHandle}</p>
                </div>
              </div>
              ...
              <button
                type="button"
                onClick={() => fileInputRef.current?.click()}
                className="px-3 py-1.5 bg-[#23252d] hover:bg-[#2c2f3a] text-gray-200 hover:text-white text-xs font-semibold rounded-xl border border-[#323542] transition-colors flex items-center space-x-1.5 cursor-pointer"
              >
                <Upload className="w-3.5 h-3.5 text-[#00ff73]" />
          ...
          {isHandleTaken && (
            <div className="flex items-center space-x-2 bg-red-500/10 border border-red-500/30 p-2.5 rounded-xl text-red-400 text-xs">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                value={name}
                onChange={(e) => {
                  // Strictly strip emojis from Display Name
                ...
                className="w-full bg-[#1c1d24] border border-[#2c2f3a] focus:border-[#00ff73] rounded-xl px-3.5 py-2 text-sm text-white placeholder-gray-500 outline-none transition-colors"
              />
            </div>
            ...
            <div>
              ...
              <div className="relative flex items-center">
                <span className="text-gray-500 absolute left-3 text-sm font-semibold">@</span>
                <input
                  type="text"
                  required
                  value={handle}
                  ...
                  className={`w-full bg-[#1c1d24] border rounded-xl pl-8 pr-3 py-2 text-sm text-white placeholder-gray-500 outline-none transition-colors ${
                    isHandleTaken ? 'border-red-500 text-red-400' : 'border-[#2c2f3a] focus:border-[#00ff73]'
                  }`}
                />
              </div>
            </div>
          ...
          <div>
            ...
            <div className="flex items-center space-x-1.5 bg-[#1c1d24] border border-[#2c2f3a] rounded-xl p-1.5">
              {STATUS_EMOJIS.map((emoji) => (
                <button
                  key={emoji}
                  type="button"
                  onClick={() => setStatusEmoji(emoji)}
                  className={`flex-1 py-1.5 rounded-lg text-base hover:scale-115 transition-transform ${
                    statusEmoji === emoji ? 'bg-white/20 shadow-xs' : 'opacity-60 hover:opacity-100'
                  }`}
                >
                  {emoji}
                </button>
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-300 uppercase tracking-wider mb-1.5 flex items-center space-x-1.5">
              <Palette className="w-3.5 h-3.5 text-[#00ff73]" />
              <span>Profile Accent Theme</span>
            </label>
            <div className="flex items-center space-x-3 bg-[#1c1d24] border border-[#2c2f3a] p-2 rounded-xl">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-[#1c1d24] border border-[#2c2f3a] focus:border-[#00ff73] rounded-xl px-3.5 py-2 text-sm text-white placeholder-gray-500 outline-none resize-none transition-colors"
            />
          </div>
            ...
            <div className="flex items-center space-x-2">
              {PRESET_AVATARS.map((url, i) => (
                <div
                  ...
                  className={`relative w-8 h-8 rounded-full overflow-hidden cursor-pointer border-2 transition-all ${
                    avatar === url ? 'border-[#00ff73] scale-110 shadow-neon-sm' : 'border-transparent opacity-60 hover:opacity-100'
                  }`}
                >
